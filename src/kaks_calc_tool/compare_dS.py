...
def scatter(ground_truth_data,fracminhash_data,ksize,wd,gene_length,p_mut,kaks_calc_method):
    ...
    kaks_calc_data = kaks_calc_data[kaks_calc_data['Method']==kaks_calc_method].tail(-1).reset_index()[['Sequence','Ks']]

    #read in fracminhash dataframe
    fracminhash_data = pd.read_csv(fracminhash_data)

    #choose column names depending on the data
    column_fracminhash_data = 'dS_constant' #USING SOURMASH
    
    column_kaks_calc_data = 'Ks'
    ...
    """
    x_pos=0.0006
    plt.text(x_pos,0.0045,f'RMSE={round(RMSE,4)}')
    plt.text(x_pos,0.0043,f'MAE={round(MAE,4)}')
    plt.text(x_pos,0.0041,f'pearson r={round(pcorr[0],4)},p={round(pcorr[1])}')
    plt.text(x_pos,0.0039,f'R^2={round(R2,4)}')
    plt.text(x_pos,0.0037,f'n={n}')
    """
    tmp = [results[column_kaks_calc_data].min(), results[column_kaks_calc_data].max()]
    plt.plot(tmp, tmp, linestyle='--')
    #plt.ylim(0,0.005)
    
    plt.title(f'dS real protein-coding sequence scaled=1 p={p_mut} ksize={ksize} len(nt)={gene_length}')
    plt.ylabel('fmh dS\nsourmash containment')
    plt.xlabel(f'{kaks_calc_method} dS\nKa/Ks calculator')

    plt.savefig(f'{wd}Ks_fmh_constant_and_{kaks_calc_method}_{p_mut}_{ksize}_{gene_length}_r2_MAE_RMSE_outliers_rm.png',bbox_inches='tight')
