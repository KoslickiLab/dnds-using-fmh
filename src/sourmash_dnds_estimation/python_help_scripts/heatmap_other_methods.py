...
folder1="HIT000324409_pairwise_take_2_with_CDS_transcripts_change_names"
folder2="HIT000324409_pairwise_take_2_with_CDS_transcripts_change_names"
...
kaks_calc = pd.read_csv(f'/data/jzr5814/kaks_calc_tool_analysis/{folder1}/kaks.axt.kaks',sep='\t')
...
fmh_dnds = pd.read_csv(f'/data/jzr5814/sourmash_dnds_estimation/tests/results/dnds_ground_truth/{folder2}/all_dnds_max_containments.csv',sep=',').rename(columns={'ksize': 'Method', 'sequence_comparison': 'Sequence'}).pivot(index='Sequence',columns='Method')[['dNdS_ratio_constant']]
...
plt = sns.heatmap(df1, vmin=0, vmax=2,xticklabels=True, cmap='seismic',yticklabels=1)
plt.set_title(title)
