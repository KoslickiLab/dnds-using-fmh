...
folder='HIT000324409_pairwise_take_2_with_CDS_transcripts_change_names'

#read in pivot tables
methods = ['GNG','GY-HKY','LPB','LWL','NG','YN']

kaks_calc = pd.read_csv(f'/data/jzr5814/kaks_calc_tool_analysis/{folder}/kaks.axt.kaks',sep='\t')
...
ksizes = [5,7,10,15,20]
#fmh_dnds = pd.read_csv(f'/data/jzr5814/sourmash_dnds_estimation/tests/results/dnds_ground_truth/{folder}/dnds_constant_all.csv',sep=',').rename(columns={'ksize': 'Method', 'B': 'Sequence'}).pivot_table(index='Sequence',columns='Method')[['dNdS_ratio_constant']]
fmh_dnds = pd.read_csv(f'/data/jzr5814/sourmash_dnds_estimation/tests/results/dnds_ground_truth/{folder}/dnds_constant_all.csv',sep=',').rename(columns={'ksize': 'Method', 'sequence_comparison': 'Sequence'}).pivot(index='Sequence',columns='Method')[['dNdS_ratio_constant']]
...
#evola_dnds = pd.read_csv('/data/jzr5814/sourmash_dnds_estimation/tests/data/evola_data/NFAS/HIT000324409/dnds_HIT000324409.csv',sep='\t')[['Seq.2','dN/dS']].rename(columns={'Seq.2': 'Sequence','dN/dS': 'modified_dNdS'})
#evola_dnds_method = ['modified_dNdS']

#merge dataframes
#df1 = pd.merge(kaks_calc, fmh_dnds, 'left', on = ["Sequence"]).merge(evola_dnds, 'left', on = ["Sequence"])
df1 = pd.merge(kaks_calc, fmh_dnds, 'left', on = ["Sequence"])
df1.to_csv(f'/data/jzr5814/sourmash_dnds_estimation/tests/results/dnds_ground_truth/{folder}/dNdS_methods_pairwise_without_mean_without_evola.csv')
print(df1)
...
for ax in fig.get_axes():
    ...
    ax.set_xticks(ticks=ind,labels=['','GNG','GY-HKY','LPB','LWL','NG','YN','k5','k7','k10','k15','k20',''],fontsize=15)
    ax.set_ylim(0,1)
...
fig.savefig(f'/data/jzr5814/sourmash_dnds_estimation/tests/results/dnds_ground_truth/{folder}/dNdS_methods_pairwise_without_mean_without_evola.png',bbox_inches='tight')
