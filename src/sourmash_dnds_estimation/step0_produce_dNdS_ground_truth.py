    ...
    for line in lines:
        if line[0] == ">":
            ...
            for p in mutation_p_list:
                ...
                dNdS_report.append([ref_names, p, dN, dS, dNdS, selection])

            GROUND_TRUTH_REF_OUTPUT.write('>'+ref_names+'\n')
            GROUND_TRUTH_REF_OUTPUT.write(ref_seq+"\n")

    #save report to csv output file
    pd.DataFrame(dNdS_report, columns=['ref_sequence_name', 'mutation_rate_p','dN','dS','dNdS_ground_truth','selection']).to_csv(GROUND_TRUTH)
