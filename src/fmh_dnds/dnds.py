...
def calc_P_nt_mut(nt_containment,k):
    """
    Returns probability for a mutation in a nucleotide sequence.
...
def calc_P_no_mut(nt_containment,k):
    """
    Returns probability for no mutation in the codon sequence.
    nt_containment: The containment index between two nucleotide sequences (this is a float)
    k: Identify the ksize used to produce containment index (this is an integer)
    """
    nt_k = 3*k
...
def calc_PdN_6_frame_translation(protein_containment,k):
    """
    Returns PdN from a six frame translation.
    protein_containment: The containment index between two protein sequences (this is a float)
    k: Identify the ksize used to produce containment index (this is an integer)
    """
    PdN = (6/k)*(1-protein_containment**(1/k))
    return(PdN)

def calc_PdS_6_frame_translation(protein_containment,nt_containment,k):
    """
    Returns synonymous mutation rate between two protein sequences.
    nt_containment: The containment index between two nucleotide sequences (this is a float)
    protein_containment: The containment index between two protein sequences (this is a float)
    ...
    """
    nt_k = 3*k
    PdS = 1-(6/k)*(1-protein_containment)-(nt_containment)**(3/nt_k)
    return(PdS)

def dNdS_ratio_6_frame_translation(protein_containment,nt_containment,k):
    """
...
def report_dNdS(nt_containment_df,prot_containment_df,ksize):
    ...
    df['ksize'] = ksize
    #apply function
    df['p_nt_mut'] = (calc_P_nt_mut(nt_containment=df['DNA_Cfrac'],k=df['ksize']))
    df['p_no_mut'] = (calc_P_no_mut(nt_containment=df['DNA_Cfrac'],k=df['ksize']))
...
def report_dNdS_6frame(nt_containment_df,prot_containment_df,ksize):
    """
    Returns dataframe of dNdS reports between all pairwise estimations of protein coding sequences.
    Uses dNdS_ratio() function to estimate dN/dS ratio.
    nt_containment_df: dataframe with prepared containment indexes of nucleotide sequences that is 
    ...
    df['p_nt_mut'] = (calc_P_nt_mut(nt_containment=df['DNA_Cfrac'],k=df['ksize']))
    df['p_no_mut'] = (calc_P_no_mut(nt_containment=df['DNA_Cfrac'],k=df['ksize']))
    df['PdN_6frame'] = (calc_PdN_6_frame_translation(protein_containment=df['AA_Cfrac'],k=df['ksize']))
